...
class BaseDocumentLayout(models.TransientModel):
    ...
    def _default_report_footer(self):
        ...
        footer_fields = [
            field
            for field in [company.phone, company.email, company.website, company.vat]
    ...
    paperformat_id = fields.Many2one(
        related="company_id.paperformat_id", readonly=False
    )
    ...
    layout_background = fields.Selection(
        related="company_id.layout_background", readonly=False
    )
    layout_background_image = fields.Binary(
        related="company_id.layout_background_image", readonly=False
    )
    ...
    def extract_image_primary_secondary_colors(
        self, logo, white_threshold=225, mitigate=175
    ):
        """
    ...
    def _compile_scss(self, scss_source):
        ...
        try:
            ...
            return (
                Markup(compiled_css)
                if isinstance(compiled_css, Markup)
                else compiled_css
            )
